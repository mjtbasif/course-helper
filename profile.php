...
<?php 
	...
	if(mysqli_num_rows($result)==1){
		...
		//Assignment
		$sql="SELECT * FROM submisson WHERE sid='$email'";
			...
			if($userType=="teacher")
			{
				...
				if ($resultA->num_rows > 0) {
				    ...
				    while($rowA = $resultA->fetch_assoc()) {
				    	...
				    	$sql="SELECT * FROM submission WHERE aid='$aid'";
				    	$res=mysqli_query($conn, $sql);
				    	if ($res->num_rows > 0){
				    		while($data = $res->fetch_assoc()) {
				    			echo '<div class="courseBox">';
									echo '<a href="'.$data["fid"].'"><h2>Download Submission</h2></a>';
									echo '<h3>submitted by '.$data["sid"].'</h3>';
								echo '</div>';
							}
				    	}
				    }
				...
				$sql="SELECT * FROM enroll WHERE sid='$email'";
				$resultC=mysqli_query($conn, $sql);
				if ($resultC->num_rows > 0) {
				    // output data of each row
				    while($rowC = $resultC->fetch_assoc()) {
				    	...
				    	$sql="SELECT * FROM course WHERE fid='$cid'";
				    	$res=mysqli_query($conn, $sql);
				    	if ($res->num_rows > 0){
				    		while($data = $res->fetch_assoc()) {
				    			echo '<div class="courseBox">';
									echo '<a href="course.php?id='.$data["fid"].'"><h2>'.$data["code"].': '.$data["name"].'</h2></a>';
									echo '<h3>instructed by '.$data["instructor"].'</h3>';
									echo '<h4>'.$data["semester"].'</h4>';
								echo '</div>';
							}
				    	}
				    }
				...
				$sql="SELECT * FROM submission WHERE sid='$email'";
				$resultA=mysqli_query($conn, $sql);
				    	...
				    	if ($res->num_rows > 0){
				    		while($data = $res->fetch_assoc()) {
				    			echo '<div class="courseBox">';
									...
									echo '<a href="'.$fid.'"><h2>Download Submission</h2></a>';
									echo '<h3>submitted on '.$data["code"].': '.$data["name"].'</h3>';
